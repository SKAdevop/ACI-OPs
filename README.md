...
User is prompted to enter URL, username/password if not entered in .env file.
User is then prompted to specify if querying for m (multiple) or s (single) interface.

This code is tested with python 3.12
APIC version 5.3(2d)
For required modules, see requirements.txt file.
